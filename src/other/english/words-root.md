...
第一部分 通过词缀认识单词
    ...
    abduct 诱拐 绑架（ab+duct引导→引走→诱拐）
		abuse 滥用 虐待（ab+use用→用坏→滥用）
    记忆口诀： 异常的洗手导致宴会缺席，原来是被绑架了，然后进行虐待
    ...
    abstain 弃权 戒绝（abs+tain拿住→不再拿住→戒绝）
    记忆口诀：可怜的人禁欲搞抽象，导致被切除最后弃权不搞抽象

		abscond 潜逃（abs+cond藏→藏起来→潜逃）
		abscind 分开 废除（abs+cind剪切→切掉→废除）

		
	3、ab-,ac-,ad-,af-,ag-,an-,ap-,ar-,as-,at-等加在同辅音字母的词根前，表示"一再"等加强意
		
		accelerate 加速（ac+celer速度+ate→一再增加速度）
    accumulate 积累（ac+cumul堆积+ate→堆积起来→积累）
		accent 强调，重音，口音（ac+cent唱歌→一再唱出→强调）
